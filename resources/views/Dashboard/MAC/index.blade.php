...
<div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
  <h2 class="text-title-md2 font-bold text-black">Dashboard MAC</h2>
  {{-- <a href="/dashboard/users/create" class="mb-2 bg-primary inline-flex items-center justify-center rounded-md px-4 py-3 text-center font-medium text-white hover:bg-opacity-90 lg:px-6 xl:px-6">Create New MAC</a> --}}
</div>
  ...
  @endif
{{-- Start Searching --}}
  <form class="max-w-md mb-3" action="/dashboard/macs" method="GET">
    @csrf   
                  ...
                  @foreach ($macs as $mac)
                      <tr>
                          ...
                          <td class="border-b border-[#eee] px-4 py-5">
                              <p class="text-black">{{ $mac->institusi }}</p>
                          </td>
                          <td class="border-b border-[#eee] px-4 py-5">
                              <p class="text-black">{{ $mac->nama }}</p>
                          </td>
                          <td class="border-b border-[#eee] px-4 py-5">
                              <p class="text-black">{{ $mac->usia }}</p>
                          </td>
                          <td class="border-b border-[#eee] px-4 py-5">
                              <p class="text-black">{{ $mac->no_telp }}</p>
                          </td>
                          <td class="border-b border-[#eee] px-4 py-5">
                              <p class="text-black">{{ $mac->email }}</p>
                          </td>
                          <td class="border-b border-[#eee] px-4 py-5">
                              <p class="text-black">{{ $mac->nim }}</p>
                          </td>
                          <td class="border-b border-[#eee] px-4 py-5">
                              <p class="text-black">{{ $mac->uname }}</p>
                          </td>
                          <td class="border-b border-[#eee] px-4 py-5">
                              <p class="text-black">{{ $mac->link }}</p>
                          </td>
                          <td class="border-b border-[#eee] px-4 py-5">
                              <div class="flex items-center space-x-3.5">
                                  <button class="hover:text-primary">
                                      <a href="/dashboard/macs/{{ $mac->id }}/edit">
                                  ...
                                  <form action="/dashboard/macs/{{ $mac->id }}" method="post">
                                      @method('delete')
                                      @csrf
                                      <button class="hover:text-primary">
