...
@section('container')
            ...
            <div class="col-lg-8">
              <form method="post" action="/dashboard/users/{{ $user->id }}">
                ...
                <input id="name" name="name" type="text" placeholder="Name" class="mb-2 w-full rounded-lg border-[1.5px] bg-transparent px-5 py-3 font-normal text-black outline-none transition focus:border-primary @error('name') border-red-500 @enderror" autofocus value="{{ old('name', $user->name) }}"/>
                @error('name')
                ...
                <input id="email" name="email" type="email" placeholder="Email" class="mb-2  w-full rounded-lg border-[1.5px] bg-transparent px-5 py-3 font-normal text-black outline-none transition focus:border-primary @error('email') border-red-500 @enderror" value="{{ old('email', $user->email) }}"/>
                @error('email')
                <div class="text-red-500">
                    {{ $message }}
                </div>
