    ...
    <div class="flex flex-wrap justify-center items-center align-middle text-center p-8 w-full">
        ...
        <div class="w-full flex justify-center items-center align-middle flex-wrap gap-8">
            <div class="bg-gray-900 border-2 border-[#d61525] font-ltmuseum-reg rounded text-white p-4 w-full md:w-[400px]">
                <h1 class="text-2xl font-ltmuseum-bold">Order ID: {{ $merchPreorder->id }}</h1>
                <p class="w-full text-left my-1">Email: {{ $merchPreorder->email }}</p>
                <p class="w-full text-left my-1">Phone: {{ $merchPreorder->phone }}</p>
                <p class="w-full text-left my-1">Line: {{ $merchPreorder->line }}</p>
                <p class="w-full text-left my-1">Subtotal: Rp
                    {{ number_format($merchPreorder->cumulative_price, 0, ',', '.') }}</p>
                <p class="w-full text-left my-1">Status: {{ $merchPreorder->status }}</p>
                <p class="w-full text-left my-1">Payment Image: </p>
                <img src="{{ asset('storage/' . $merchPreorder->payment_image) }}" alt="" class="w-48">
                <p class="w-full text-left my-1">Items: </p>
                @foreach ($merchPreorder->preorderDetails as $orderDetail)
