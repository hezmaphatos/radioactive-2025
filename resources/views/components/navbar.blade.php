<div id="navbar" x-data="{ isOpen: false }"
    class="font-royale-serif sticky top-0 z-50 w-full bg-[#20661c] backdrop-blur-md shadow-md py-8 px-4 lg:px-12 xl:px-20">
                ...
                @foreach ($menus as $menu)
                    <div class="relative group flex flex-col items-center text-white">
                        <a href="{{ $menu['href'] }}" class="group hover:text-[#f6e79c] transition duration-300 text-xl">
                            <i class="fas {{ $menu['icon'] }}
                                {{ (Request::is('/') && $menu['href'] == '/') || Request::is(ltrim($menu['href'], '/')) ? 'text-[#f6e79c]' : 'text-white' }}
                                transform group-hover:scale-125 group-hover:text-[#f6e79c] transition duration-300"></i>
                        </a>
                        <div
    ...
    <div x-show="isOpen" @click.away="isOpen = false"
        ...
        @foreach ($menus as $menu)
            @php
                $isActive = (Request::is('/') && $menu['href'] == '/') || Request::is(ltrim($menu['href'], '/'));
            @endphp
            <div class="flex items-center space-x-3 mb-4">
                <i class="fas {{ $menu['icon'] }} w-5 h-5 {{ $isActive ? 'text-[#f6e79c]' : 'text-white' }}"></i>
                <a href="{{ $menu['href'] }}"
                    class="text-sm font-ltmuseumbold no-underline {{ $isActive ? 'text-[#f6e79c]' : 'text-white' }}">
                    {{ $menu['desc'] }}
                </a>
            </div>
        @endforeach
